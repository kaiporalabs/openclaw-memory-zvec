...
async function listMarkdownFiles(rootDir, relPrefix) {
...
async function readStagedBackfillIds(stmDir) {
    let entries = [];
    try {
...
export async function runRemBackfill(params) {
    ...
    if (params.stageShortTerm !== false) {
        ...
        for (const chunk of allChunks) {
            ...
            await fsp.writeFile(path.join(stmDir, fileName), `${JSON.stringify({
    ...
    const lines = allChunks.slice(0, 40).map((c) => `- [${BACKFILL_SOURCE}] ${c.relPath}:${c.startLine}-${c.endLine} ${c.text.trim().replace(/\s+/g, " ").slice(0, 280)}`);
...
//# sourceMappingURL=rem-backfill.js.map