...
export function isShortTermMemoryPath(relPath: string): boolean {
...
function buildEntryKey(result: Pick<MemorySearchResult, "path" | "startLine" | "endLine">): string {
  return `${normalizeMemoryPath(result.path)}:${Math.max(1, Math.floor(result.startLine))}:${Math.max(1, Math.floor(result.endLine))}`;
}

function mergeQueryHashes(existing: string[], next: string): string[] {
...
function mergeRecentDistinct(existing: string[], day: string, max: number): string[] {
  if (!day) {
    return existing;
  }
...
async function readStore(workspaceDir: string, nowIso: string): Promise<ShortTermRecallStore> {
  try {
    ...
    const parsed = JSON.parse(raw) as ShortTermRecallStore;
...
function totalSignalCount(entry: ShortTermRecallEntry): number {
  return (
    Math.max(0, Math.floor(entry.recallCount)) +
...
export async function recordShortTermRecalls(params: {
  ...
  for (const result of relevant) {
    ...
    const recallCount = Math.max(
      0,
      Math.floor(existing?.recallCount ?? 0) + (dedupeSignal ? 0 : 1),
    );
...
function matchChunkForEntry(entry: ShortTermRecallEntry, chunks: ChunkRow[]): ChunkRow | null {
  const direct = chunks.find(
    (chunk) =>
      normalizeMemoryPath(chunk.relPath) === entry.path &&
...
export async function rankRecallPromotionCandidates(params: {
  ...
  const store = await readStore(workspaceDir, nowIso);
  const maxAgeDays =
    typeof params.config.maxAgeDays === "number" ? params.config.maxAgeDays : undefined;
  const candidates: DreamingPromotionCandidate[] = [];
  ...
  for (const entry of Object.values(store.entries)) {
    ...
    const recency = clampScore(
      calculateRecencyComponent(ageDays, params.config.recencyHalfLifeDays),
    );
