...
import type { MemoryZvecStatusSelfTest } from "./memory-manager.js";
...
function kvBlock(rows: [string, string][], labelWidth = 22): string {
  const w = Math.max(labelWidth, ...rows.map(([a]) => a.length));
  return rows.map(([k, v]) => `  ${padLabel(k, w)}  ${v}`).join("\n");
}

function truncatePath(p: string | undefined, max = 76): string {
  if (p == null || p === "") return "—";
...
function formatSelfTest(st: MemoryZvecStatusSelfTest): string {
  ...
  const sql = st.sqlite;
  const sqlLine =
    sql.ok && typeof sql.files === "number" && typeof sql.chunks === "number"
  ...
  lines.push(
    st.memoryCorpusRootsPresent.length > 0
 ...
 */
export function formatMemoryStatusHuman(status: MemoryProviderStatus): string {
  const custom = status.custom as
    | {
        zvecDataRoot?: string;
        memoryZvecStatusSelfTest?: MemoryZvecStatusSelfTest;
  ...
  const ftsErr =
    fts && typeof fts === "object" && "error" in fts && fts.error ? String(fts.error) : "";
  const ftsLine = [
